...
class SmallDemoDataset_DiffusionPolicy(Dataset):  # Load everything into memory
    def __init__(self, data_path, obs_process_fn, obs_space, include_rgb, include_depth, device, num_traj):
        ...
        print(f"include_rgb: {self.include_rgb}, include_depth: {self.include_depth}") 
    ...
    def __getitem__(self, index):
        ...
        # # debug
        # print("obs_seq.keys()",obs_seq.keys())
        # print("obs_seq.keys()",obs_seq['rgb'].shape)
        # exit(0)
        return {
...
def evaluate_model(args, run_name, envs, device):
    # 加载最佳检查点
    ...
    if os.path.exists(checkpoint_dir):
        ...
        if checkpoint_files:
            ...
            # 创建评估用的agent
            eval_agent = Agent(envs, args).to(device)
...
if __name__ == "__main__":
    ...
    video_prefix = "test_videos" if args.test_mode else "train_videos"
    envs = make_eval_envs(
        args.env_id,
        args.num_eval_envs,
        args.sim_backend,
        env_kwargs,
        other_kwargs,
        video_dir=f"runs/{run_name}/{video_prefix}" if args.capture_video else None,
    ...
    # ---------------------------------------------------------------------------- #
    if args.test_mode:
        eval_metrics = evaluate_model(args, run_name, envs, device)
        envs.close()
        writer.close()
        exit(0)
